    ...
    <header className="fixed inset-x-0 top-0 z-50 px-3 pt-3 sm:px-6 sm:pt-4 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        >
          ...
          <NavLink
            to="/"
            className="relative flex h-14 w-[140px] shrink-0 items-center sm:w-[180px]"
          >
            <img
              src="/logo_sem_fundo.png"
              alt="Arrive"
              className={`absolute left-0 h-9 object-contain transition-all duration-700 sm:h-10 ${
                scrolled
                  ? "scale-75 opacity-0"
            ...
            <img
              ...
              alt=""
              aria-hidden="true"
              className={`absolute left-0 h-12 w-12 object-contain transition-all duration-500 sm:h-14 sm:w-14 ${
                scrolled
          ...
          <nav className="hidden items-center gap-9 lg:flex">
            {NAV_ITEMS.map(({ to, label }) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `relative py-1 text-sm font-medium tracking-wide transition-colors duration-300 after:absolute after:-bottom-0.5 after:left-0 after:h-[2px] after:w-full after:origin-left after:bg-current after:transition-transform after:duration-300 ${
                    isActive
                      ? "after:scale-x-100"
        ...
        >
          <nav
            id={menuId}
            aria-hidden={!open}
            className="rounded-2xl border border-black/5 bg-white/95 p-3 shadow-lg backdrop-blur-xl"
          >
